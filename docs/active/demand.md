...
| **Honest-empty gate over-fires on grounded input** (surfaced by E-019) — survey 67% / expand 33% false-negative abstention on real demand; tighten the abstention threshold so it fires only on genuinely thin input. | **High** (the consistency promise, P3; honest-empty is the IA-4 gate the whole shelf leans on) | ~1 block | **ready** — pull string in `work/T-019-02/findings.md`. |
