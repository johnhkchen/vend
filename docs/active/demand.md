...
| **`expand-fragment`** (`epic/E-016.md`) — the demand-extraction primitive: a rough fragment (felt "this is rough", a one-liner, a TODO) → a board-ready, priced signal. Vend articulates *for* you — edit a draft, don't compose from blank. Upstream of ProposeEpic; the primitive the articulation lift reuses. | **Keystone** (the articulation lift's foundation; O1 gap 0.72; *not* trust-gated — ships while the wallet is parked, and accrues the E1 data that ungates it) | ~1 block | **active → E-016** (S-016-01, 2 tickets: pure core + read-never-invent gate → register + `vend expand` gesture). From the articulation-cost batch (#1). Lined up for the loop. |
