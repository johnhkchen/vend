...
| **Survey play** (`epic/E-017.md`) — the run-0 bootstrap: `vend survey` reads a rough project (repo + docs + state) → a ranked, staged demand board of the highest-leverage moves it sees. `expand-fragment` at project scale; *"articulating is exhausting" → "review a list."* (IA-3/4.) | **High** (the cold-start half of the experience; 2nd brick of automating STEER; *not* trust-gated) | ~1 block | **active → E-017** (S-017-01, 2 tickets: pure core — ranked `Signal[]` + read-never-invent/honest-empty → register + `vend survey` gesture). From the articulation-cost batch (#2). Reuses E-016's `Signal`; pre-fill budget generously (E-016's 211k lesson). Lined up for the loop. |
