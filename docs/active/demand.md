...
| **SteerProject-lite** (`epic/E-018.md`) — `vend steer` reads a project → a ranked board **and** the real **forks** (the genuine decisions only a human can make), staged for assent. Productizes `project-steering.md`'s 10 moves; composes E-016 + E-017 and adds fork-surfacing. The steer capstone — *the fruit-picker itself*. | **High** (the steer capstone; 3rd/last brick of automating STEER; collapses the human to author+assent, `clearing-dynamics.md`; *not* trust-gated) | ~1–2 blocks | **active → E-018** (S-018-01, 2 tickets: pure core — `Fork` type + read-never-invent/**fork-genuineness** gate → register + `vend steer` gesture). From the articulation-cost batch (#3). Reuses E-017's board half; budget > Survey's 300k. Lined up for the loop. |
