...
| **Measured envelopes** (`epic/E-013.md`) — read the run log's actuals; bound each play's cost at the value-tier percentile; surface it as the Confirm default so *accept-the-default* is earned, not guessed. The recalibration core (IA-12…IA-15). | **Keystone** (the budget contract made honest; the first rung of the 2-hour mechanic) | ~2-3h | **active → E-013** (S-013-01, 3 tickets — telemetry + pure `recalibrate` core + hierarchical bias-corrected calibration, IA-16: *"we overestimate by ~80%"*, generic-play × project dataset). In the loop. Design settled in `information-architecture.md`. |
